...
namespace VRStandardAssets.ShootingGallery
{
    ...
    public class ShootingGalleryGun : MonoBehaviour
    {
        ...
        [SerializeField]
        private ProgressbarAmmo progressbarAmmo;
        ...
        private void Start()
        {
            targetsLayerMask = 1 << LayerMask.NameToLayer("Targets");
        }
        ...
        //        m_Damping * (1 - Mathf.Exp(k_DampingCoef * Time.deltaTime)));
            ...
            Vector3 p;
            if (Physics.Raycast(mouseRay, out hit, 1000, targetsLayerMask))// Targets))
            {
                p = hit.point;
            }
            else
            {
                p = mouseRay.GetPoint(30);
            }
            Quaternion lookAtRotation = Quaternion.LookRotation(p - m_GunContainer.position);
            ...
            if (Input.GetMouseButton(0) && !firing && 0 < progressbarAmmo.AmmoCount )
            {
                StartCoroutine(Fire());
            }
        }
        ...
        //    Transform target = null;// shootingTarget ? shootingTarget.transform : null;
        ...
        private IEnumerator Fire()//Transform target)
        {
            ...
            if (Physics.Raycast(m_GunEnd.position, m_GunEnd.forward, out hit, 1000))
            {
                var target = hit.transform.GetComponent<ShootingTarget>();
                if (target != null)
                {
                    target.Hit(m_GunEnd.forward);
                }
            }
            //Debug.DrawLine(m_GunEnd.position, m_GunEnd.position + m_GunEnd.forward *100, Color.cyan, 55);
            //if (target)                lineLength = Vector3.Distance (m_GunEnd.position, target.position);
